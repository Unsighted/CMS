...
class GestorArticulos{
	...
	public function mostrarImagenController($datos){
		...
		if($ancho < 800 || $alto < 400){
			...
			$aleatorio = mt_rand(100, 999); // Numero aleatorio para el nombre de la imágen.
			$ruta = "../../views/images/articulos/temp/articulo".$aleatorio.".jpg";
			$origen = imagecreatefromjpeg($datos); // Creamos la imágen a partir de la imágen temporal.
			//$destino = imagecrop($origen, ["x"=>0, "y"=>0, "width"=>800, "height"=>400]); // Recortamos la imágen.
			imagejpeg($origen, $ruta); // Exportar la imágen en la carpeta tmp.
	...
	public function guardarArticuloController(){
		...
		if(isset($_POST["tituloArticulo"])){
			...
			$borrar = glob("views/images/articulos/temp/*"); // Borra el contenido de la carpeta tmp de artículos.
			...
			$origen = imagecreatefromjpeg($imagen); // Original sin recortar.
	...
	public function editarArticuloController(){
		...
		if(isset($_POST["editarTitulo"])){
			...
			if(isset($_FILES["editarImagen"]["tmp_name"])){
				...
				$origen = imagecreatefromjpeg($imagen); // Convertir imágen a jpeg.

				$destino = imagecrop($origen, ["x"=>0, "y"=>0, "width"=>800, "height"=>400]);

				imagejpeg($destino, $ruta); // Crear la nueva imágen recortada.
			...
			if($ruta == ""){
				...
				$ruta = $_POST["fotoAntigua"]; // Ruta de la imágen antigua.
			...
			// Cuando no se cambie la imágen y se oprima guardar se enviarán a la base los datos ya existentes.
