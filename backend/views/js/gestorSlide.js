...
$("#columnasSlide").on("drop", function(e){
	...
	if(Number(imagenSize) > 2000000){
		...
		$("#columnasSlide").before('<div class="alert alert-warning alerta text-center">El archivo excede el peso permitido, 10mb</div>')
		...
		$.ajax({
			...
			success: function(respuesta){
				...
				if(respuesta == 0){
					...
					$("#columnasSlide").before('<div class="alert alert-warning alerta text-center">La imagen es inferior a 800px * 400px</div>')
...
$(".eliminarSlide").click(function(){
	...
	idSlide = $(this).parent().attr("id");  // Capturamos el valor del elemento padre span.
	rutaSlide = $(this).attr("ruta"); 			// Capturamos el valor de la ruta del elemento padre span.

	$(this).parent().remove();						 // Removemos el elemento span(ocultar la imágen de la vista cuadrada).
	$("#item"+idSlide).remove();					// Removemos el elemento item(ocultar la imágen de la vista del editor).
	...
	$.ajax({ // Borramos la imágen de la DB.
...
$(".editarSlide").click(function(){
	...
	// Mostrar el contenido en el span al tocar el lápiz.
	...
	$("#guardar"+idSlide).click(function(){
		...
		enviarId = idSlide.slice(4); // Quitamos primeros 4 caracteres los cuales son(item).
...
$("#ordenarSlide").click(function(){
	...
	$("#columnasSlide").sortable({ 			// Al mover la imágen desencadenar estos eventos.
		revert: true,                     // Volver a su posición en cso de que no quiera arrastrar la imágen.
		connectWith: ".bloqueSlide",			// Conecta con todos los li a través de esta clase.
		handle: ".handleImg",							// Toma la imágen con la clase .handleImg.
		stop: function(event){

			for(var i= 0; i < $("#columnasSlide li").length; i++){ // Contamos el total de li.

				almacenarOrdenId[i] = event.target.children[i].id;  // target: #columnasSlide, children[i]:li, Obtengo el id de cada li.
				ordenItem[i]  =  i+1; 															// posición de cada indice + 1.
